...
begin
  ...
  puts "Workflow show_account_balances completed successfully!"
  puts "Final Result:"
  pp JSON.pretty_generate(result)
  puts "Running workflow show_account_transactions with input:"
  final_data[:url] = "#{final_data[:base_url]}/sandbox/v5/consents?bic=#{final_data[:bic]}&app-id=#{final_data[:app_id]}"
  final_data[:booking_status] = "both"
  final_data[:consent_id] = final_data[:create_consent_response]["consentId"]
  final_data[:dateFrom] = (Date.today - 30).to_s
  final_data[:dateTo] = Date.today.to_s
  #
  # :show_account_transactions
  #
  result = Navesti.run(:show_account_transactions, final_data)
  puts "Workflow show_account_transactions completed successfully!"
  puts "Final Result:"
  pp JSON.pretty_generate(result)
rescue => e
